...
<main class="container">
    <div class="p-4 p-md-5 mb-4 text-white rounded bg-dark">
        <div class="col-md-6 px-0">
                @php $article = getArticles()[0]; @endphp
                @if(isset(getArticles()[0]))
                    <h1 class="display-4 fst-italic">{{$article->title}}</h1>
                    <p class="lead my-3">{{\Str::limit($article->short_text, 100)}}</p>
                    <p class="lead mb-0"><a href="{{$article->slug}}" class="text-white fw-bold">Продолжить чтение...</a></p>
                @else
    ...
    <div class="row mb-2">
        <div class="col-md-6">
            <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                <div class="col p-4 d-flex flex-column position-static">
                    @php $articleTop1 = getArticles()[0]; @endphp
                    @if(isset(getArticles()[0]))
                        <strong class="d-inline-block mb-2 text-primary">World</strong>
                        <h3 class="mb-0">{{$articleTop1->title}}</h3>
                        <div class="mb-1 text-muted">{{\Carbon\Carbon::parse($articleTop1->created_at)->format('M d')}}</div>
        ...
        <div class="col-md-6">
            <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                <div class="col p-4 d-flex flex-column position-static">
                    @php $articleTop2 = getArticles()[1]; @endphp
                    @if(isset(getArticles()[1]))
                        <strong class="d-inline-block mb-2 text-primary">World</strong>
                        <h3 class="mb-0">{{$articleTop2->title}}</h3>
                        <div class="mb-1 text-muted">{{\Carbon\Carbon::parse($articleTop2->created_at)->format('M d')}}</div>
